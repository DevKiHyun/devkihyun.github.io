...
잠시 머신러닝 얘기로 돌아오도록 하죠. 우리는 앞에서 분류모델을 *'입력값 X를 넣어주면 모델에서 Y가 나올 확률'*  이라고 표현했습니다. 근데 Y가 나올 확률이기 때문에 Y는 확률 변수라고 정했는데, 입력값 X은 뭐라고 해야 될지 애매했습니다. 벌써 눈치채신 분들도 있겠지만 이제 우리는 조건부 확률을 가지고서 분류모델을 부족함 없이 표현할 수 있게 되었습니다. 바로 단서가 입력값 X가 되는 것입니다. A를 제공했을 때 B가 나올 확률 <=> 입력값 X를 제공했을 때 Y가 나올 확률! 기호로 모델을 표현하면 P(Y|X) 로 나타낼 수 있습니다.
