...
확률 변수 $$ X $$ 는 사건(event)를 특정 실수인 $$ x $$ 로 매핑을 해줍니다. 그러고 나서 그 $$ x $$ 를 확률 P에 넣어주면 바로 해당 사건에 대한 가능성을 나타내는 확률값이 나오게 됩니다. 따라서 확률 $$ P $$ 는 확률변수를 입력으로 받는 또 하나의 __함수__ 인 것입니다.

### 확률 분포 (Probability Distribution)

우리는 사건(event)을 확률변수 $$ X $$ 를 통해 특정값들로 매핑해준 뒤 그 특정값 $$ x $$ 를 확률 $$ P $$ 에 넣어 $$ P(x) $$, 즉 확률값을 얻는다는 것을 이해했습니다. 확률 분포는 이 확률변수 $$ X $$ 에 대한 확률값들이 어떻게 분포되어 있는지를 말해줍니다. 그림을 보시면 바로 이해되실겁니다.
