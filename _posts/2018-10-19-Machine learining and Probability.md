...
앞서 말했듯이 데이터 쌍 X,Y는 사건(event)이고 조건부확률 $$ P(Y \mid X) $$ 의 $$X, Y$$ 는 확률 변수입니다. 따라서 데이터 X,Y를 바로 확률변수 $$ X,Y $$ 로 취급해서 넣으시면 잘못된 표현이 됩니다. 개념상 확률분포는 사건(event)이 아닌 __매핑된 수치__ 를 입력값으로 받아드립니다. 그러나 실제로는 데이터들은 특정 수치를 갖고 있는 경우가 많습니다. 예를 들어 28x28 크기의 사진이 있다고 하면, 이 사진을 사건으로 생각해도 되지만 0부터 255 사이의 정수값을 갖는 픽셀들이 28x28 넓이로 구성되어는 숫자로 여길 수도 있습니다. 따라서 데이터를 그냥 확률분포의 입력값으로 취급하고 넘어가도 크게 문제되지 않는 것입니다. 만약 엄격하게 명시하고 간다면 데이터 X,Y 에 포함되는 특정값 x,y 가 있다고 하면 

$$ P(Y=y \mid X=x) $$

라고 표현할 수 있습니다. 엄격하게 얘기해야 되는 상황에선 제대로 표기하시면 되고 이번 시간에는 설명의 편의를 위해 $$ P(Y \mid X) $$ 를 쓰도록 하겠습니다.
