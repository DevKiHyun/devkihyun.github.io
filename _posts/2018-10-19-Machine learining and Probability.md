...
Y는 확률 변수입니다. 그 확률변수 Y에 대해 실제 확률 분포(모르지만)는 $$ f_{r}(Y) $$ 이고 우리가 가정한 확률 분포는 $$ f_{h}(Y) $$ 입니다. $$ f_{h}(Y) $$ 를 구성하는 $$ \theta $$ 는 $$ \mu $$ 와 $$ \sigma  $$ 입니다. y 라는 특정 값에 대해서 $$ f_{h}(y) $$ 는 __Likelihood__ 이고 $$ f_{r}(y) $$ 는 __확률__ 입니다.
